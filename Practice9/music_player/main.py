...
def draw_controls_legend(screen, font, x, y):
    ...
    for i, (key, action) in enumerate(controls):
        ...
        action_surf = font.render(f"  {action}", True, COLOR_WHITE)

        screen.blit(key_surf, (col, row))
        screen.blit(action_surf, (col + key_surf.get_width(), row))
...
def main():
    ...
    while running:
        ...
        if player.get_total_tracks() > 0:
            ...
            counter_text = f"Track {player.current_index + 1} / {player.get_total_tracks()}"
            counter_surf = font_info.render(counter_text, True, COLOR_GRAY)
            screen.blit(counter_surf, counter_surf.get_rect(center=(SCREEN_WIDTH // 2, 152)))

            pos = player.get_position_seconds()
            draw_progress_bar(screen, 70, 172, SCREEN_WIDTH - 140, 14, pos, COLOR_ACCENT, COLOR_DARK_GRAY)
